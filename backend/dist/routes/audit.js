...
// Get all audit plans
router.get('/plans', auth_1.authenticateToken, async (req, res) => {
    try {
        const { year, status } = req.query;
        let query = (0, database_1.default)('audit_plans')
            .select('audit_plans.*', 'creator.first_name as creator_first_name', 'creator.last_name as creator_last_name', 'approver.first_name as approver_first_name', 'approver.last_name as approver_last_name')
...
router.post('/plans', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('director', 'senior_auditor'), async (req, res) => {
    try {
        const { title, description, year } = req.body;
        const userId = req.user.id;
        const [planId] = await (0, database_1.default)('audit_plans').insert({
            title,
            description,
            year,
            created_by: userId,
...
router.get('/engagements', auth_1.authenticateToken, async (req, res) => {
    try {
        const { status, year, plan_id } = req.query;
        let query = (0, database_1.default)('audit_engagements')
            .select('audit_engagements.*', 'lead.first_name as lead_first_name', 'lead.last_name as lead_last_name', 'audit_plans.title as plan_title', 'risk_assessments.risk_level')
...
router.post('/engagements', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('director', 'senior_auditor'), async (req, res) => {
    try {
        const { title, objective, scope, audit_plan_id, risk_assessment_id, lead_auditor, planned_start_date, planned_end_date, budgeted_hours } = req.body;
        // Generate engagement number (format: YYYY-NNN)
        const year = new Date().getFullYear();
        const countResult = await (0, database_1.default)('audit_engagements')
        ...
        const [engagementId] = await (0, database_1.default)('audit_engagements').insert({
            engagement_number,
            title,
            objective,
            scope,
...
router.get('/engagements/:id/working-papers', auth_1.authenticateToken, async (req, res) => {
    try {
        const engagementId = req.params.id;
        const workingPapers = await (0, database_1.default)('working_papers')
            .select('working_papers.*', 'creator.first_name as creator_first_name', 'creator.last_name as creator_last_name', 'reviewer.first_name as reviewer_first_name', 'reviewer.last_name as reviewer_last_name')
...
router.post('/engagements/:id/working-papers', auth_1.authenticateToken, async (req, res) => {
    try {
        const engagementId = req.params.id;
        const { title, description, wp_reference } = req.body;
        const userId = req.user.id;
        const [wpId] = await (0, database_1.default)('working_papers').insert({
...
router.get('/engagements/:id/findings', auth_1.authenticateToken, async (req, res) => {
    try {
        const engagementId = req.params.id;
        const findings = await (0, database_1.default)('audit_findings')
            .select('audit_findings.*', 'assigned.first_name as assigned_first_name', 'assigned.last_name as assigned_last_name')
...
router.post('/engagements/:id/findings', auth_1.authenticateToken, async (req, res) => {
    try {
        const engagementId = req.params.id;
        const { title, condition, criteria, cause, effect, recommendation, severity, assigned_to, target_date } = req.body;
        // Generate finding number
        const countResult = await (0, database_1.default)('audit_findings')
            .where('engagement_id', engagementId)
            .count('* as count')
            .first();
        const sequence = (Number(countResult?.count) || 0) + 1;
        const finding_number = `F${sequence.toString().padStart(2, '0')}`;
        const [findingId] = await (0, database_1.default)('audit_findings').insert({
            engagement_id: engagementId,
            finding_number,
            title,
            condition,
